...
public class AnonNode extends GMixTool {
	...
	// some settings
	public int PUBLIC_PSEUDONYM; // this anon node will be registered under this pseudonym at the
									// info-service; the pseudonym can be used by other anon nodes
									// for addressing
	...
	public boolean LAYER_1_LINKS_MESSAGES; // when true, layer one will tag messages of the same
											// user with a pseudonym (this is required by some
											// plug-ins; e.g. some recoding schemes will transmit
											// symmetrically encrypted messages after an initial
											// hybridly encrypted message; the recoder plug-in needs
											// the pseudonym to choose the right decryption key in
											// that example)
	...
	public AnonNode(XMLResource settings, ToolName toolName) {
		System.out.println("AnonNode initializing... (" + toolName.name() + ")");

		this.gMixTool = toolName;
		...
		System.out.println("AnonNode up (" + toolName.name() + ": " + this.PUBLIC_PSEUDONYM + ")");
	}
	...
	private void initAndRegisterAtInfoService(XMLResource settings) {
		...
		this.infoService = new InfoServiceClient(settings.getPropertyAsInetAddress("infoService/address"),
				settings.getPropertyAsInt("infoService/port"));

		if (gMixTool == ToolName.MIX || gMixTool == ToolName.P2P)
		...
		if (!LOCAL_MODE_ON) {
			System.err
					.println("WARNING: LOCAL_MODE is set to FALSE; Mixes and Information Service will be available via Network; Only safe for trusted networks!");
			if (settings.getPropertyAsString("mixBindAddress").equalsIgnoreCase("AUTO"))
				try {
					settings.addTemporaryValue("mixBindAddress", InetAddress.getLocalHost().getHostAddress());
				} catch (UnknownHostException e) {
					e.printStackTrace();
					throw new RuntimeException("could not detect ip address!");
				}
		} else {
			settings.addTemporaryValue("mixBindAddress", "localhost");
		}

		if (settings.getPropertyAsString("mixBindPort").equalsIgnoreCase("AUTO")) {
			int port = infoService.getSuggestedPort();
			settings.addTemporaryValue("mixBindPort", "" + port);
		}

		this.IS_DUPLEX = settings.getPropertyAsBoolean("infoService/duplex");
		this.IS_RELIABLE = settings.getPropertyAsBoolean("infoService/reliable");
		this.IS_CONNECTION_BASED = settings.getPropertyAsBoolean("infoService/connectionBased");
		this.IS_ORDER_PRESERVING = settings.getPropertyAsBoolean("infoService/orderPreserving");
		this.REPLY_DETECTION_ON = settings.getPropertyAsBoolean("globalReplyDetectionOn");
		this.RS_DEBUG_OUTPUT_ON = settings.getPropertyAsBoolean("globalRsDebugOutputOn");
		this.ROUTING_MODE = RoutingMode.getMode(settings);
		this.EXPECTED_NUMBER_OF_USERS = settings.getPropertyAsInt("numberOfUsers");
		this.CRYPTO_PROVIDER = settings.getPropertyAsString("cryptoProvider");
		this.NUMBER_OF_THREADS = settings.getPropertyAsInt("numberOfThreads");
		this.MAX_PAYLOAD = settings.getPropertyAsInt("maxPayload");
		this.QUEUE_BLOCK_SIZE = settings.getPropertyAsInt("queueBlockSize");
		this.RECORD_STATISTICS_ON = settings.getPropertyAsBoolean("globalRecordStatisticsOn");
		MixMessage.recordStatistics = RECORD_STATISTICS_ON;
		this.DISPLAY_ROUTE_INFO = settings.getPropertyAsBoolean("globalDisplayRouteInfo");
		this.LAYER_1_LINKS_MESSAGES = settings.getPropertyAsBoolean("globalLayer1LinksMessages");
		this.SOCKET_MIX_BACKEND_QUEUE_SIZE = settings.getPropertyAsInt("socketMixBackendQueueSize");
		this.SERVER_SOCKET_QUEUE_SIZE = settings.getPropertyAsInt("globalServerSocketQueueSize");
		this.SERVER_SOCKET_BACKLOG = settings.getPropertyAsInt("globalServerSocketBacklog");
		this.TIME_TO_WAIT_FOR_FURTHER_DATA = settings
				.getPropertyAsInt("timeToWaitForFurtherData");
		this.LAYER_4_CLIENT_INPUT_STREAM_REPLY_BUFFER_SIZE = settings
				.getPropertyAsString("layer4ClientInputStreamReplyBufferSize");
		this.LAYER_4_MIX_INPUT_STREAM_REQUEST_BUFFER_SIZE = settings
				.getPropertyAsString("layer4MixInputStreamReplyBufferSize");

		this.DEBUG_MODE_ON = infoService.getIsDuplexModeOn();
		this.NUMBER_OF_MIXES = infoService.getNumberOfMixes();
		this.FREE_ROUTE_LENGTH = settings.getPropertyAsInt("globalFreeRouteLength");
		...
		if (gMixTool == ToolName.CLIENT) {
			...
			throw new RuntimeException("not supported tool type specified: " + gMixTool);
		}

		if (IS_MIX) {
			infoService.postValueAsMix(PUBLIC_PSEUDONYM, "ADDRESS",
					settings.getPropertyAsInetAddress("mixBindAddress").getAddress());
			infoService.postValueAsMix(PUBLIC_PSEUDONYM, "PORT",
					Util.intToByteArray(settings.getPropertyAsInt("mixBindPort")));
			this.sockets = new HashMap<Integer, AdaptiveAnonServerSocket>();
			this.requestInputQueue = new ArrayBlockingQueue<Request[]>(
					settings.getPropertyAsInt("globalRequestInputQueueSize"));
			this.requestOutputQueue = new ArrayBlockingQueue<Request[]>(
					settings.getPropertyAsInt("globalRequestOutputQueueSize"));
			this.replyInputQueue = new ArrayBlockingQueue<Reply[]>(
					settings.getPropertyAsInt("globalReplyInputQueueSize"));
			this.replyOutputQueue = new ArrayBlockingQueue<Reply[]>(
					settings.getPropertyAsInt("globalReplyOutputQueueSize"));
		}
	...
	private void setReferencesBetweenComponents() {
		for (Controller c : components)
			c.setComponentReferences(networkLayerMix, networkLayerClient, recodingLayerMix, recodingLayerClient,
					outputStrategyLayerMix, outputStrategyLayerClient, transportLayerMix, transportLayerClient,
					applicationLayerMix, applicationLayerClient);
	}

	private void generateComponents() {
		// NOTE: the order of the items is important (do not change it)
		if (IS_MIX) {
			this.outputStrategyLayerMix = new Layer3OutputStrategyMixController(this, settings, userDatabase, clock,
					infoService);
			this.recodingLayerMix = new Layer2RecodingSchemeMixController(this, settings, userDatabase, clock,
					infoService);
			...
			this.applicationLayerMix = new Layer5ApplicationMixController(this, settings, userDatabase, clock,
					infoService);
		}
		if (IS_CLIENT) { // the lower layer client plug-ins are loaded at runtime when a socket is
							// created (directly or through layer 5 plug-ins)
			this.outputStrategyLayerClient = new Layer3OutputStrategyClientController(this, settings, userDatabase,
					clock, infoService);
			this.recodingLayerClient = new Layer2RecodingSchemeClientController(this, settings, userDatabase, clock,
					infoService);
			this.networkLayerClient = new Layer1NetworkClientController(this, settings, userDatabase, clock,
					infoService);
			this.transportLayerClient = new Layer4TransportClientController(this, settings, userDatabase, clock,
					infoService);
			this.applicationLayerClient = new Layer5ApplicationClientController(this, settings, userDatabase, clock,
					infoService);
		}
	}
	...
	public StreamAnonServerSocket createStreamAnonServerSocket(int bindPort, CommunicationMode communicationMode,
			boolean isFreeRoute) {
		...
		return new StreamAnonServerSocketImpl(this, PUBLIC_PSEUDONYM, bindPort, communicationMode, isFreeRoute);

	}

	public DatagramAnonServerSocket createDatagramServerSocket(int bindPort, CommunicationMode communicationMode,
			boolean isReliable, boolean isOrderPreserving, boolean isFreeRoute) {
		...
		return new DatagramAnonServerSocketImpl(this, PUBLIC_PSEUDONYM, bindPort, communicationMode, isReliable,
				isOrderPreserving, isFreeRoute);
	}

	public ConnectedDatagramAnonServerSocket createConnectedDatagramServerSocket(int bindPort,
			CommunicationMode communicationMode, boolean isReliable, boolean isOrderPreserving, boolean isFreeRoute) {
		...
		return new ConnectedDatagramAnonServerSocketImpl(this, PUBLIC_PSEUDONYM, bindPort, communicationMode,
				isReliable, isOrderPreserving, isFreeRoute);
	}
	...
	public DatagramAnonSocket createDatagramSocket(CommunicationMode communicationMode, boolean isReliable,
			boolean isOrderPreserving, boolean isFreeRoute) {
		if (!IS_CLIENT)
			throw new RuntimeException("only clients/p2p mixes can open none-server sockets");
		return new DatagramAnonSocketClientImpl(this, communicationMode, isReliable, isOrderPreserving, isFreeRoute);
	}

	public StreamAnonSocket createStreamSocket(CommunicationMode communicationMode, boolean isFreeRoute) {
		if (!IS_CLIENT)
			throw new RuntimeException("only clients/p2p mixes can open none-server sockets");
		return new StreamAnonSocketClientImpl(this, communicationMode, isFreeRoute);
	}

	public ConnectedDatagramAnonSocket createConnectedDatagramSocket(CommunicationMode communicationMode,
			boolean isReliable, boolean isOrderPreserving, boolean isFreeRoute) {
		return new ConnectedDatagramAnonSocketClientImpl(this, communicationMode, isReliable, isOrderPreserving,
				isFreeRoute);
	}

	// called by layer 3 (output strategy) mix plug-ins
	public void putOutRequest(Request request) {
		if (!isFinalHop(request)) { // put request in output queue, from where it will be sent to
									// the next hop (via layer 1)
			...
			int dstPort = Util.byteArrayToShort(Arrays.copyOf(request.getByteMessage(), 2)); // get
																								// desired
																								// socket
	...
	public void putOutRequests(Request[] requests) {
		if (ROUTING_MODE == RoutingMode.CASCADE) {
			if (!IS_LAST_MIX) // put data in request output queue, from where it will be forwarded
								// to the next mix via the layer 1 plug-in
				putInRequestOutputQueue(requests);
			else
				// forward data to the responsive socket
