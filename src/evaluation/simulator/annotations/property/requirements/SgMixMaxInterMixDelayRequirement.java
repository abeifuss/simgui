...
public class SgMixMaxInterMixDelayRequirement extends Requirement {
	
	
@Override
public boolean check() {
	SimPropRegistry gcr = SimPropRegistry.getInstance();
	SimProp thisSimprop = gcr.getProperties().get("SGMIX_MAX_INTER_MIX_DELAY");
	SimProp dependSimprop = gcr.getProperties().get("SGMIX_MIN_INTER_MIX_DELAY");
	
	Integer valuethisSimpProp = (Integer)thisSimprop.getValue();
	Integer valuedependSimprop = (Integer)dependSimprop.getValue();
	
	String msg = "Depedency Violation: Must be equal or greater than \"SGMIX_MIN_INTER_MIX_DELAY\"";
