...
public class SgMixMinClientMixDelayRequirement extends Requirement {
	
	
@Override
public boolean check() {
	SimPropRegistry gcr = SimPropRegistry.getInstance();
	SimProp thisSimprop = gcr.getProperties().get("SGMIX_MIN_CLIENT_MIX_DELAY");
	SimProp dependSimprop = gcr.getProperties().get("SGMIX_MAX_CLIENT_MIX_DELAY");
	
	Integer valuethisSimpProp = (Integer)thisSimprop.getValue();
	Integer valuedependSimprop = (Integer)dependSimprop.getValue();

	String msg = "Depedency Violation: Must be equal or less than \"SGMIX_MAX_CLIENT_MIX_DELAY\"";
