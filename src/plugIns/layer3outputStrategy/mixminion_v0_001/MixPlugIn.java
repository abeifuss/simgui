...
public class MixPlugIn extends Implementation implements Layer3OutputStrategyMix {
	...
	public void constructor() {
		...
		this.DEFAULT_POOL_SIZE = settings.getPropertyAsInt("mixminionDefaultPoolSize");
		this.INTERVAL = settings.getPropertyAsInt("mixminionInterval");
		this.MIN_POOL = settings.getPropertyAsInt("mixminionMinPool");
		this.SEND_RATE = settings.getPropertyAsFloat("mixminionSendRate");
		this.MIN_SEND = settings.getPropertyAsInt("mixminionMinSend");
		this.requestPool = new SimplexTimedDynamicPool(true);
		this.replyPool = new SimplexTimedDynamicPool(false);
		try {
			this.secureRandom = SecureRandom.getInstance(settings.getPropertyAsString("mixminionPrngAlgorithm"));
